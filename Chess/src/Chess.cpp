...
// print only the board to screen
...
void Chess::displayBoard() const
{
    ...
    if (!s_recommendation.empty()) {
        cout << s_recommendation << "\n";
    }
...
void Chess::setCodeResponse(int codeResponse)
{
    if (((11 <= codeResponse) && (codeResponse <= 13)) ||
        (codeResponse == 21) || (codeResponse == 31) ||
        (codeResponse == 41) || (codeResponse == 42))
    {
        m_codeResponse = codeResponse;
    }
}

// Initialize the static recommendation string to empty
string Chess::s_recommendation = "";
